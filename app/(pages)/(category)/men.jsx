  ...
  useEffect(() => {
    if (productsData) {
      ...
      dataArr = dataArr?.filter(
        (ele) =>
          ele?.category?.name?.toLowerCase()?.includes("men") ||
          ele?.subcategory?.name?.toLowerCase()?.includes("men") ||
          ele?.title?.toLowerCase()?.includes("men")
      );
      setmenProductsData(dataArr);
    }
  }, [productsData]);
    ...
    >
      ...
      <View style={[glStyles.containerInner, { gap: 10 }]}>
        ...
        >
          ...
          <ThemedText style={{ fontWeight: 700, fontSize: 20 }}>Men</ThemedText>
          <View></View>
        </View>
        <SearchInput />
        <ScrollView
        ...
        >
          {menProductsData && menProductsData?.length ? (
            <ProductGrid
              data={menProductsData}
              numColumns={2}
              spacing={12}
              renderItem={({ item }) => (
                <Pressable
          ...
          ) : menProductsData && menProductsData?.length < 1 ? (
            <ProductsUnavailable
