  ...
  useEffect(() => {
    if (productsData) {
      ...
      dataArr = dataArr?.filter(
        (ele) =>
          ele?.category?.name?.toLowerCase()?.includes("women") ||
          ele?.subcategory?.name?.toLowerCase()?.includes("women") ||
          ele?.title?.toLowerCase()?.includes("women")
      );
      setWomenProductsData(dataArr);
    }
  }, [productsData]);
  ...
  return (
    ...
    >
      ...
      <View style={[glStyles.containerInner, { gap: 10 }]}>
        ...
        >
          {womenProductsData && womenProductsData?.length ? (
            <ProductGrid
          ...
          ) : womenProductsData && womenProductsData?.length < 1 ? (
            <ProductsUnavailable
